  ...
  useEffect(() => {
    if (selectedEventId) {
      ...
      if (socket) {
        ...
        socket.on('checkin:update', handleLiveUpdate);

        return () => {
          socket.emit('leave_event_room', selectedEventId);
          socket.off('checkin:update', handleLiveUpdate);
        };
      }
    }
  }, [selectedEventId, token, socket]);
